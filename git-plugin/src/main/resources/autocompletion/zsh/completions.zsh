function _autocomplete_branch() {
    ...
    desc=("${(@f)$(RET_ENV=ZSH_AUTOCOMPLETE ret git autocomplete git-branch --word="$word" --repository="${repository_flag_value}" ${ignore_context_aware_flag})}")
    vals=( ${desc%%:*} )
...
function _autocomplete_repository() {
    local word=${words[$CURRENT]}
    if _matches_flag_syntax "$word"; then return; fi
    desc=("${(@f)$(RET_ENV=ZSH_AUTOCOMPLETE ret git autocomplete git-repository --word="$word")}")
    vals=( ${desc%%:*} )
    compadd -d desc -aQU vals
    compstate[insert]=menu # no expand
}
...
function _autocomplete_repository_flag() {
    ...
    desc=("${(@f)$(RET_ENV=ZSH_AUTOCOMPLETE ret git autocomplete git-repository --word="${repository_flag_value}")}")
    vals=( ${desc%%:*} )
...
function _autocomplete_pullrequest() {
    ...
    _contains_any_flag '-ica' '--ignore-context-aware' && local ignore_context_aware_flag='-ica'
    local repository_flag_value=${RET_COMBINED_OPT_ARGS[-r]-${RET_COMBINED_OPT_ARGS[--repository]}}
    candidates=("${(@f)$(RET_ENV=ZSH_AUTOCOMPLETE ret git autocomplete git-pullrequest --word="$word" --repository="${repository_flag_value}" ${ignore_context_aware_flag})}")
    desc=( ${candidates#*:} )
    vals=( ${candidates%%:*} )
    compadd -d desc -aQU vals
    compstate[insert]=menu # no expand
