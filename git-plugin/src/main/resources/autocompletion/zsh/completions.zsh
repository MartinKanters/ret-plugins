...
function _autocomplete_pipeline() {
    ...
    _contains_any_flag '-ica' '--ignore-context-aware' && local ignore_context_aware_flag='-ica'
    local repository_flag_value=${RET_COMBINED_OPT_ARGS[-r]-${RET_COMBINED_OPT_ARGS[--repository]}}
    desc=("${(@f)$(RET_ENV=ZSH_AUTOCOMPLETE ret git autocomplete pipeline --word="${word}" --repository="${repository_flag_value}" ${ignore_context_aware_flag})}")
    vals=( ${desc%%:*} )
...
function _autocomplete_pipeline_run() {
    ...
    _contains_any_flag '-ica' '--ignore-context-aware' && local ignore_context_aware_flag='-ica'
    local repository_flag_value=${RET_COMBINED_OPT_ARGS[-r]-${RET_COMBINED_OPT_ARGS[--repository]}}
    desc=("${(@f)$(RET_ENV=ZSH_AUTOCOMPLETE ret git autocomplete pipeline-run --pipeline-id="${pipeline_id}" --word="$word" --repository="${repository_flag_value}" ${ignore_context_aware_flag})}")
    vals=( ${desc%%:*} )
